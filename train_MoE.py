...
from models import NodeGCNMoE
from models import NodeGINMoEmlp
from models import NodeGINMoEself
from models import NodeGATMoElin
from models import NodeGATMoEAtt
...
parser.add_argument('--base_model', default='GCN', type=str, required=True)
parser.add_argument('--num_experts', default=4, type=int)
parser.add_argument('--top_k', default=2, type=int)
parser.add_argument('--shared_expert', default=True, type=lambda x: x.lower() == 'true')
parser.add_argument('--aux_lambda', default=1e-2, type=float)
...
def main():
    ...
    print(f'dataset {args.dataset_name} | task node | num_classes {out_dim} | '
            f'model {args.model} | shared expert {args.shared_expert} | seed {args.seed} | split_idx 0')


    # ----- Model ----- #
    if args.model == 'GCN_MoE_lin':
        model = NodeGCNMoE(
            in_dim, hidden_dim, out_dim,
            num_experts=args.num_experts,
            top_k=args.top_k,
            shared_expert=args.shared_expert,
            capacity_factor=1.0, drop_tokens=False,
            dropout=0.2
        ).to(device)

    elif args.model == 'GIN_MoE_mlp':
        model = NodeGINMoEmlp(
            in_dim, hidden_dim, out_dim,
            learn_eps=True,
            num_experts=args.num_experts,
    ...
    elif args.model == 'GIN_MoE_self':
        model = NodeGINMoEself(
            in_dim, hidden_dim, out_dim, 
            num_experts=args.num_experts, 
            capacity_factor=1.0, 
            drop_tokens=False, 
    ...
    if args.sched == 'cosine':
        ...
        sched_warm = LinearLR(optimizer, start_factor=0.1, total_iters=warm)
        sched_main = CosineAnnealingLR(optimizer, T_max=main, eta_min=args.lr_min)
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        optimizer.zero_grad()
        logits, aux = model(data, return_aux=True, aux_mask=train_mask)
        loss = criterion(logits[train_mask], data.y[train_mask]) + args.aux_lambda * aux
        loss.backward()
        optimizer.step()
        ...
        model.eval()
        with torch.no_grad():
            logits, val_aux = model(data, return_aux=True, aux_mask=val_mask)
        ...
        if epoch % 10 == 0 or epoch == args.epochs:
            ...
            print(f'\rEpoch {epoch}/{args.epochs} | lr {cur_lr:.2e} | loss {loss:.4f} | aux {aux:.4f} | '
                    f'val {val_acc:.4f}/{val_f1:.4f} | val_aux {val_aux:.4f} | ',end='')
    ...
    fname = f"{args.model}_{args.shared_expert}_{args.dataset_name}_{args.seed}.pt"
    ...
    with torch.no_grad():
        ...
        logits = model(data, return_aux=False)
        pred = logits.argmax(dim=-1)
        ...
        print(f'\n[results] Test Acc {test_acc:.4f} | F1 {test_f1:.4f}')

    # ----- Save results ----- #
    result_dir = f'./results/expert{args.num_experts}'
    os.makedirs(result_dir, exist_ok=True)
    result_csv_path = os.path.join(result_dir, f'{args.base_model}_results.csv')
    if not os.path.exists(result_csv_path):
        pd.DataFrame(columns=['model','shared_expert','params','seed','dataset','acc','f1']).to_csv(result_csv_path, index=False)
