...
class GCNMoEConv(GCNConv):
    def __init__(self, in_dim, out_dim, num_experts=4, shared_expert=False, top_k=2, capacity_factor=1.0, drop_tokens=False, **kwargs):
        super().__init__(in_dim, out_dim, **kwargs)

        self.lin = MoELinearTopk(in_dim, out_dim, 
    ...
    @property
    def aux(self):
        return getattr(self.lin, 'aux', torch.tensor(0.0, device=next(self.parameters()).device))
    



class NodeGCNMoE(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, num_experts=4, top_k=2, shared_expert=False, capacity_factor=1.0, drop_tokens=False, dropout=0.5):
        super().__init__()

        self.g1 = GCNMoEConv(in_dim, hidden_dim, num_experts=num_experts, top_k=top_k,
                             shared_expert=shared_expert,
                             capacity_factor=capacity_factor,
                             drop_tokens=drop_tokens)
        
        self.g2 = GCNMoEConv(hidden_dim, hidden_dim, num_experts=num_experts, top_k=top_k,
                             shared_expert=shared_expert,
                             capacity_factor=capacity_factor,
                             drop_tokens=drop_tokens)
    ...
    def forward(self, data, return_aux=False, aux_mask=None):
        ...
        self.g1.lin._aux_mask = aux_mask
        self.g2.lin._aux_mask = aux_mask
        ...
        if return_aux:
            ...
            self.g1.lin._aux_mask = None
            self.g2.lin._aux_mask = None
